...
//! `[build-dependencies] odni-schemas = ...` cannot build with tampered
//! schemas.
//!
//! The check is opt-out via `--no-default-features` (the `verify-on-build`
//! feature is enabled by default). Disable only if you understand what
//! you're giving up — typically a CI environment that runs the same
//! verification more thoroughly out of band.
...
fn main() {
    if env::var_os("CARGO_FEATURE_VERIFY_ON_BUILD").is_none() {
        // Feature disabled by consumer; skip verification.
        return;
    }
    ...
    // Re-run only when the manifest changes. The crate version pin in the
    // consumer's Cargo.lock is the integrity boundary; we don't want to
    // re-hash 14k files on every Marque rebuild.
    ...
    let manifest_digest_expected = std::fs::read_to_string(&digest_path)
        .unwrap_or_else(|e| {
            panic!(
                "odni-schemas: cannot read manifest.sha256 at {}: {}",
                digest_path.display(),
                e
            )
        })
    ...
    if manifest_digest_actual != manifest_digest_expected {
        panic!(
            "odni-schemas: manifest.sha256 does not match SHA-256 of manifest.txt.\n\
             expected: {}\n\
             actual:   {}\n\
    ...
    // 2. Verify every line of the manifest against the file on disk.
    ...
    if !errors.is_empty() {
        ...
        panic!(
            "odni-schemas: integrity check failed for {} of {} files.\n\
             First few:\n  {}{}\n\
    ...
    println!(
        "cargo:warning=odni-schemas: verified {} files against manifest (digest {})",
        checked,
