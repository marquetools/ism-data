...
//! Build-time integrity check for the ITS-MS package.
//!
...
//! `[build-dependencies] ism-its-ms = ...` cannot build with tampered
//! schemas.
//!
...
fn main() {
    ...
    if env::var_os("ISM_DATA_SKIP_VERIFY").is_some() {
        println!(
            "cargo:warning=ism-its-ms: integrity check SKIPPED (ISM_DATA_SKIP_VERIFY set)"
        );
        return;
    }
    ...
    let manifest_bytes = std::fs::read(&manifest_path).unwrap_or_else(|e| {
        panic!(
            "ism-its-ms: cannot read manifest.txt at {}: {}",
    ...
    let manifest_digest_expected = std::fs::read_to_string(&digest_path)
        .unwrap_or_else(|e| {
            panic!(
                "ism-its-ms: cannot read manifest.sha256 at {}: {}",
                digest_path.display(),
                e
            )
        })
    ...
    if manifest_digest_actual != manifest_digest_expected {
        panic!(
            "ism-its-ms: manifest.sha256 does not match SHA-256 of manifest.txt.\n\
             expected: {}\n\
             actual:   {}\n\
    ...
    if !errors.is_empty() {
        ...
        panic!(
            "ism-its-ms: integrity check failed for {} of {} files.\n\
             First few:\n  {}{}\n\
    ...
    println!(
        "cargo:warning=ism-its-ms: verified {} files against manifest (digest {})",
        checked,
