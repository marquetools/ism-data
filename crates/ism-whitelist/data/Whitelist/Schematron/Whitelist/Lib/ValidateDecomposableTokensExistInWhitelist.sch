...
-->
<sch:pattern xmlns:sch="http://purl.oclc.org/dsdl/schematron" abstract="true" id="ValidateDecomposableTokensExistInWhitelist">
    ...
    <sch:rule id="ValidateDecomposableTokensExistInWhitelist-R1" context="$context">
        <sch:assert test="( ( matches(util:getFirstItemFromSequence($list),'\*') ) or (every $searchTerm in tokenize(util:getSpaceSeparatedStringFromSequence(util:expandDecomposableTetras($searchTermList)), ' ') satisfies             some $Term in tokenize(util:getSpaceSeparatedStringFromSequence(util:expandDecomposableTetras($list)), ' ') satisfies (matches (normalize-space($searchTerm), normalize-space($Term))) )  )" flag="error" role="error">
            <sch:value-of select="$errMsg"/>  Document Value(s): [<sch:value-of select="util:getSpaceSeparatedStringFromSequence(util:expandDecomposableTetras($searchTermList))"/>]   Whitelist Value(s): [<sch:value-of select="util:getCommaSeparatedStringFromSequence($list)"/>]
