...
//! Build-time integrity check for the DHZMC-TDF package.
//!
...
//! `[build-dependencies] ism-dhzmc-tdf = ...` cannot build with tampered
//! schemas.
//!
...
fn main() {
    ...
    if env::var_os("ISM_DATA_SKIP_VERIFY").is_some() {
        println!(
            "cargo:warning=ism-dhzmc-tdf: integrity check SKIPPED (ISM_DATA_SKIP_VERIFY set)"
        );
        return;
    }
    ...
    let manifest_bytes = std::fs::read(&manifest_path).unwrap_or_else(|e| {
        panic!(
            "ism-dhzmc-tdf: cannot read manifest.txt at {}: {}",
    ...
    let manifest_digest_expected = std::fs::read_to_string(&digest_path)
        .unwrap_or_else(|e| {
            panic!(
                "ism-dhzmc-tdf: cannot read manifest.sha256 at {}: {}",
                digest_path.display(),
                e
            )
        })
    ...
    if manifest_digest_actual != manifest_digest_expected {
        panic!(
            "ism-dhzmc-tdf: manifest.sha256 does not match SHA-256 of manifest.txt.\n\
             expected: {}\n\
             actual:   {}\n\
    ...
    if !errors.is_empty() {
        ...
        panic!(
            "ism-dhzmc-tdf: integrity check failed for {} of {} files.\n\
             First few:\n  {}{}\n\
    ...
    println!(
        "cargo:warning=ism-dhzmc-tdf: verified {} files against manifest (digest {})",
        checked,
