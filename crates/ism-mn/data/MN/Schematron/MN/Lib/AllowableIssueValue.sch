...
<sch:pattern xmlns:sch="http://purl.oclc.org/dsdl/schematron" xmlns:ism="urn:us:gov:ic:ism"
   abstract="true"
   id="AllowableIssueValues">  
   <sch:p class="codeDesc" ism:classification="U" ism:ownerProducer="USA">
      This abstract pattern checks to see if a value exists in the Issue Acronym List. The following 
   ...
   <sch:rule context="$context">
      ...
      <sch:assert test="some $token in $Issue satisfies $token = $searchTerm or matches($searchTerm, concat('^',$token,'$'))"
         flag="error">
