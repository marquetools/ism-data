...
<sch:pattern id="ISM-ID-00201" xmlns:sch="http://purl.oclc.org/dsdl/schematron">
    <sch:p id="ruleText">[ISM-ID-00201][Error] Attribute displayOnlyTo must not contain values that have passed their deprecation date.</sch:p>
    ...
    <sch:rule context="*[@ism:displayOnlyTo]">
        ...
        <sch:assert id="ISM-00201"
            test="count($reportErr)=0"
            flag="error">
            [ISM-ID-00201][Error] For attribute displayOnlyTo, value(s) <sch:value-of select="$reportErr"/>
        </sch:assert>
    </sch:rule>
</sch:pattern>