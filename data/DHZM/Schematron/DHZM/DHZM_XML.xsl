...
<xsl:template match="/">
      ...
      declares some variables, and includes all of the Rule .sch files.</svrl:text>
         ...
         <svrl:active-pattern>
            ...
            <xsl:attribute name="id">DHZM-ID-00001</xsl:attribute>
            <xsl:attribute name="name">DHZM-ID-00001</xsl:attribute>
            <svrl:text>
...
<!--PATTERN DHZM-ID-00001-->


	<!--RULE DHZM-ID-00001-R1-->
<xsl:template match="*[@dhzm:DESVersion]" priority="1000" mode="M8">
      <svrl:fired-rule xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                       context="*[@dhzm:DESVersion]"
                       id="DHZM-ID-00001-R1"/>

		    <!--ASSERT warning-->
<xsl:choose>
         <xsl:when test="matches(@dhzm:DESVersion,'^202111(\-.{1,23})?$')"/>
         <xsl:otherwise>
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                                test="matches(@dhzm:DESVersion,'^202111(\-.{1,23})?$')">
            ...
            with an optional extension. The value provided was: <xsl:text/>
                  <xsl:value-of select="./@dhzm:DESVersion"/>
                  <xsl:text/>
               </svrl:text>
            </svrl:failed-assert>
         </xsl:otherwise>
