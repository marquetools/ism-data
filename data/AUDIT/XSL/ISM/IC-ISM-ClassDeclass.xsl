...
<xsl:template name="class.declass">
  ...
  <xsl:if test="($n-classification and ($n-classification != 'U') and ($n-classification != 'NU')) or
    ...
    <xsl:variable name="warning-missing-declass-info">
      <xsl:if test="$n-classification and ($n-classification != 'U') and ($n-classification != 'NU') 
        ...
        and ($n-declassdate = '') 
        and ($n-declassevent = '')">
        <xsl:text>(&#xA0;WARNING! A declassification date or event should be specified for a document with a 25X or 50X declassification exemption, unless the document has a declassification exemption of 25X1-human, 50X1-HUM, 50X2-WMD, or AEA.&#xA0;)</xsl:text>
      </xsl:if>
    </xsl:variable>
      ...
      <xsl:if test="$n-declassexception">
        <xsl:choose>
          <xsl:when test="$n-declassexception = 'AEA'">
            <xsl:text>Not Applicable to RD/FRD portions. See source list for NSI portions.</xsl:text>
          </xsl:when>
          <xsl:otherwise>
