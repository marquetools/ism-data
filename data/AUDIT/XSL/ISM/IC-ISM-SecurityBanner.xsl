        ...
        <xsl:choose>
          ...
          <xsl:when test="contains($n-ownerproducer,' ')">
            ...
            <xsl:choose>
              ...
              <xsl:otherwise><xsl:value-of select="$warn-parse-classif"/></xsl:otherwise>
            </xsl:choose>
...
<!-- **** Determine AtomicEnergyMarkings ****-->
  ...
  <xsl:if test="number($documentdate) &lt; 20090331">
    ...
    <xsl:if test="(not($portion) and
      ...
      <xsl:text>//</xsl:text><xsl:value-of select="$warn-missing-classif" />
    </xsl:if>
  </xsl:if>
...
<xsl:template name="ism:get.dissem.banner">
  ...
  <xsl:choose>
    <xsl:when test="$first">
      <xsl:if test="not(($first = 'RD' and (contains($rest,'RD-CNWDI') or contains($rest,'RD-SG'))) or ($first = 'FRD' and contains($rest,'FRD-SG')))">
        <xsl:choose>
  ...
  <xsl:if test="$rest">
    <xsl:if test="not(($first = 'RD' and (contains($rest,'RD-CNWDI') or contains($rest,'RD-SG'))) or ($first = 'FRD' and contains($rest,'FRD-SG')))">
      <xsl:choose>
        <xsl:when test="((starts-with($first,'RD-SG') or starts-with($first,'RD-CNWDI')) and starts-with($rest,'RD-SG')) or (starts-with($first,'FRD-SG') and starts-with($rest,'FRD-SG'))">
          <xsl:text>-</xsl:text>
        </xsl:when>
        <xsl:otherwise>
          <xsl:text>/</xsl:text>
...
<xsl:template name="ism:get.sar.name">
  ...
  <xsl:variable name="SAR-val" select="substring-after($name,'SAR-')" />
  ...
  <xsl:choose>
    ...
    <xsl:when test="$abbreviate = 'yes'">
      <xsl:value-of select="$SAR-val"/>
    </xsl:when>
    <xsl:otherwise>
      <xsl:choose>
        <xsl:when test="$SAR-val='ABC'">ALPHA BRAVO CHARLIE</xsl:when>
        <xsl:when test="$SAR-val='DEF'">DELTA ECHO FOX</xsl:when>
        <xsl:when test="$SAR-val='GHI'">GULF HOTEL INDIGO</xsl:when>
      </xsl:choose>
    </xsl:otherwise>
  </xsl:choose>
